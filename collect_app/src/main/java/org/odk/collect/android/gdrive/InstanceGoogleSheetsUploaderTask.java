        ...
        for (int i = 0; i < instancesToUpload.size(); i++) {
            ...
            if (isCancelled()) {
                outcome.messagesByInstanceId.put(instance.getId().toString(),
                        TranslationHandler.getString(Collect1.getInstance().getAppContext(), R.string.instance_upload_cancelled));
                return outcome;
            }
            ...
            if (forms.size() != 1) {
                outcome.messagesByInstanceId.put(instance.getId().toString(),
                        TranslationHandler.getString(Collect1.getInstance().getAppContext(), R.string.not_exactly_one_blank_form_for_this_form_id));
            } else {
                try {
