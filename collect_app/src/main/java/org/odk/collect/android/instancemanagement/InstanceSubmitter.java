...
public class InstanceSubmitter {
    ...
    public Pair<Boolean, String> submitSelectedInstances(List<Instance> toUpload) throws SubmitException {
        ...
        if (protocol.equals(TranslationHandler.getString(Collect1.getInstance().getAppContext(), R.string.protocol_google_sheets))) {
            if (PermissionUtils.isGetAccountsPermissionGranted(Collect1.getInstance().getAppContext())) {
        ...
        for (Instance instance : toUpload) {
            try {
                ...
                resultMessagesByInstanceId.put(instance.getId().toString(), customMessage != null ? customMessage : TranslationHandler.getString(Collect1.getInstance().getAppContext(), R.string.success));
                ...
                if (InstanceUploaderUtils.shouldFormBeDeleted(formsRepository, instance.getJrFormId(), instance.getJrVersion(),
                    ...
                    Collect1.getInstance().getAppContext().getContentResolver().delete(deleteForm, null, null);
                }

                String action = protocol.equals(TranslationHandler.getString(Collect1.getInstance().getAppContext(), R.string.protocol_google_sheets)) ?
                ...
                if (!submissionEndpoint.equals(TranslationHandler.getString(Collect1.getInstance().getAppContext(), R.string.default_odk_submission))) {
        ...
        return new Pair<>(anyFailure, InstanceUploaderUtils.getUploadResultMessage(instancesRepository, Collect1.getInstance().getAppContext(), resultMessagesByInstanceId));
    }
