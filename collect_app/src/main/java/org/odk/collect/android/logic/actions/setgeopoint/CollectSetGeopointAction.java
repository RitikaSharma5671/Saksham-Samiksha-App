...
public class CollectSetGeopointAction extends SetGeopointAction implements LocationListener {
    ...
    public void requestLocationUpdates() {
        ...
        if (maxAccuracyLocationClient == null) {
            maxAccuracyLocationClient = new MaxAccuracyWithinTimeoutLocationClient(new GoogleFusedLocationClient(Collect1.getInstance().getApplicationVal()), this);
        }
        ...
        if (GeneralSharedPreferences.getInstance().getBoolean(KEY_BACKGROUND_LOCATION, true)
            && new PlayServicesChecker().isGooglePlayServicesAvailable(Collect1.getInstance().getAppContext())) {
